...
export default function Post({ params }: { params: { slug: string } }) {
    ...
    return (
        <div className="flex-1 pt-24 pb-16 w-full pr-6 pl-6 lg:pr-0 lg:pl-0 lg:w-[740px] mx-auto">
            <h1 className="font-extrabold tracking-tight text-3xl lg:text-6xl mb-2">{res.title}</h1>
            <span className="text-stone-500 font-semibold">Posted on {res.date}</span>
            <div className="prose prose-lg prose-p:volkh-font prose-zinc max-w-none mt-5" dangerouslySetInnerHTML={{ __html: res.content as TrustedHTML }} />
        </div>
    );
}
